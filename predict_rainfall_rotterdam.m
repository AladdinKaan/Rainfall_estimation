close all
load('rain.mat')
Rainfall = table2array(Climate344);
Rainfall = Rainfall(29:2029);    %to make the code run faster
Rh = Rainfall;
Rd = averages(Rainfall,24);
Rw = averages(Rainfall,24*7);
Rm = averages(Rainfall,24*30);
...
%select data type:
data = Rh;
train_data = train_data_h;
test_data = test_data_h;
...
xlabel('Time')
ylabel('Rainfall (mm)')
title('Rainfall prediction on test data (hourly)')
